...
// ── Stalled job detection (Phase 30 final spec alias) ────────────────────────

/**
 * detectStalledJobs — returns only jobs that are running but stalled past threshold.
 * Subset of detectIncompleteJobs focused on running/stalled state.
 */
export async function detectStalledJobs(
  stallThresholdMinutes = 30,
): Promise<IncompleteJob[]> {
  const all = await detectIncompleteJobs(stallThresholdMinutes);
  return all.filter(j => j.status === "running");
}

/**
 * summarizeRestartState — alias for summarizeRecoveryState().
 * Phase 30 final spec uses this name.
 */
export async function summarizeRestartState(): Promise<RestartRecoverySummary> {
  return summarizeRecoveryState();
}
