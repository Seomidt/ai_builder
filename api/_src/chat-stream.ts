...
export default async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    // ── Video/lyd fallback: Gemini kunne ikke analysere mediefilen ───────────────
    // Når video/lyd er for stor (>~15 MB) eller indeholder ingen tale, returnerer
    // processDirectAttachment en placeholder med source="video_fallback".
    // I stedet for at lade den grounded AI sige "Jeg kan ikke finde det",
    // streamer vi en venlig forklaring direkte.
    const isMediaFallbackOnly = hasDocIntent && docCtx.length > 0 &&
      docCtx.every((d: any) => d.source === "video_fallback" || d.source === "video_attachment");

    if (isMediaFallbackOnly) {
      const mediaFilename = docCtx[0]?.filename ?? "mediefilen";
      const isAudio = (docCtx[0]?.mime_type ?? "").startsWith("audio/");
      const mediaLabel = isAudio ? "lydfilen" : "videoen";
      const fallbackMsg = [
        `Din ${mediaLabel} "${mediaFilename}" blev uploadet, men indholdet kunne desværre ikke analyseres automatisk.`,
        ``,
        `Dette kan skyldes:`,
        `- Filen er for stor til direkte analyse (grænse: ~15 MB)`,
        `- Filen indeholder ingen tale eller synlig tekst som kan udtrækkes`,
        ``,
        `Du er velkommen til at beskrive hvad ${mediaLabel} viser, så hjælper jeg dig videre baseret på din beskrivelse.`,
      ].join("\n");

      sendEvent({ type: "status", text: "Behandler...", routeType: "attachment_first" });
      sendEvent({ type: "token", text: fallbackMsg });
      sendEvent({ type: "done", answer: fallbackMsg, model: modelUsed, provider: providerUsed, routeType: "attachment_first", answer_source: "video_fallback_notice" });
      return safeEnd();
    }
